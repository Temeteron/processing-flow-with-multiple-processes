...
int main(int argc,char *argv[]) {
	...
	while (1) {
		...
		if(str[0] == 'q') {
			printf("q");
			sprintf(msg,"\tP1 is terminatinging\n");
			write(2,msg,strlen(msg));
			return 1;
		}

		/* calculate expressions mult, div */
		calculate(str);
		printf("%s",str);
		fflush(stdout);
		sprintf(msg,"\tP1 sends to P2\n");
		write(2,msg,strlen(msg));	
	}	
...
void calculate(char str[]) {
	int  num1, num2, num, i, j, pos, start, end, flag = 0;
